...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        url = res.url;
        str = "<div><strong>Link:</strong><span> " + res.url + "</span></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        str = "<div><strong>Link:</strong><span> " + link + "</span></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }
    ...
    var showImg = $('<img src="images/loading.gif" style="width: 100px;height: 80px;" />');
